...
namespace SolumSample.Interfaces
{
    ...
    public class Solus : IProvider
    {
        ...
        //{
        //    Console.WriteLine($"Enviando dados para o provedor Solus: {jsonData}");
        //}
        public async Task<string> SendAllItems(string jsonArticles, string company, string store, string URL)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    return responseBody;
                    //MessageBox.Show(responseBody,"OK|");
                }
                else
                {
                    string msgError = $"Falha ao enviar o artigo. Status code: {response.StatusCode}";
                    return msgError;
            ...
            {
                string msgError = $"Erro na solicitação HTTP: {ex.Message}";
                return msgError;
                //MessageBox.Show(msgError);
            }
        }
            ...
            {
                string msgError = $"Falha ao obter o token.";
                return msgError;
            }
            ...
            if (token.accessToken != null && token.refreshToken != null)
            {
                Console.WriteLine($"Token de acesso: {token.accessToken}");
                Console.WriteLine($"Token de atualização: {token.refreshToken}");
        ...
        public async Task<(string accessToken, string refreshToken)> GetToken()
        {
            ...
            try
            {
                ...
                {
                    //throw new Exception($"Falha ao obter o token. Status code: {response.StatusCode}");
                    return ("ERROR",response.StatusCode.ToString());
                }
            }
            catch (HttpRequestException ex)
            {
                throw new Exception($"Erro na solicitação HTTP: {ex.Message}");
            }
